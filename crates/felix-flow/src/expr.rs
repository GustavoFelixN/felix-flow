...
#[cfg(test)]
mod tests {
    use tests::binding_usage::BindingUsage;

    use super::*;
    use crate::{env::Env, stmt::Stmt};
    ...
    fn eval_mul() {
        assert_eq!(
            Expr::Operation {
                lhs: Box::new(Expr::Number(Number(7))),
    ...
    fn eval_div() {
        assert_eq!(
            Expr::Operation {
                lhs: Box::new(Expr::Number(Number(300))),
    ...
    fn eval_func_call() {
        ...
        env.store_func(
            ...
            Stmt::Expr(Expr::Operation {
                lhs: Box::new(Expr::BindingUsage(BindingUsage { name: "x".to_string() })),
                rhs: Box::new(Expr::BindingUsage(BindingUsage { name: "y".to_string() })),
                op: Op::Add
            })
        );
